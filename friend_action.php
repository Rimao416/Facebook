...
if(isset($_POST['action'])){
    if($_POST['action']=='send_request'){
        $data=array(':request_from_id' =>$_SESSION['user_id'],
        ':request_to_id'=>$_POST['to_id'],
        ':request_status'=>'Pending',
        ':request_notification_status' =>'No'
    );
    $query="INSERT INTO friend_request (request_from_id, request_to_id, request_status,request_notification_status) VALUES (:request_from_id, :request_to_id, :request_status, :request_notification_status)";
    $statement=$connect->prepare($query);
    $statement->execute($data);
    }
    if($_POST['action']=="count_un_seen_friend_request"){
        $query="SELECT COUNT(request_id) as Total FROM friend_request
        WHERE request_to_id='".$_SESSION["user_id"]."' AND request_status ='Pending'
   ...
   if($_POST['action']=='load_friend_request_list'){
        ...
        $query="SELECT * FROM friend_request WHERE request_to_id='".$_SESSION["user_id"]."' AND request_status='Pending' ORDER BY request_id DESC";
        $result=$connect->prepare($query);
        $result->execute();
        $output='';
        $number_req=$result->rowCount();
        if($number_req==0){
            $output .='<li style="font-size:18px"> Aucune demande </li>';
        ...
        foreach($result as $row){
            ...
            <li>
				...
				<button type="button" name="accept_friend_request_button" class="btn btn-primary btn-xs pull-right accept_friend_request_button" data-request_id="'.$row["request_from_id"].'" id="accept_friend_request_button_'.$row["request_from_id"].'"><i class="fa fa-plus" aria-hidden="true"></i> Accept</button>
			</li>
            ';
        }
        echo $output;            
	...
	if($_POST['action'] == "accept_friend_request")
	{
		...
		WHERE request_from_id = '".$_POST["request_id"]."' AND request_to_id='".$_SESSION["user_id"]."'
		";
        $connect->query($query);
    ...
    if($_POST['action']=='load_friends'){
		...
		AND friend_request.request_status = 'Confirm'  ".$condition."
        ...
        if($statement->rowCount()>0){
            ...
            foreach($statement->fetchAll() as $row){
                ...
                if($row["request_from_id"]!=$_SESSION['user_id']){
                    ...
                    $temp_user_id=$rpw["request_id_id"];
                }
                $count++;
                if($count==1){
                    $html .='<div class="row">';
